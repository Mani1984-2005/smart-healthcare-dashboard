...
import { StatusBadge, PageHeader, ActionButton } from "../components/ui";
...
export default function ComplaintsPage({ darkMode, addToast }) {
  ...
  function handleSubmit(e) {
    ...
    const newComplaint = { ...form, id: Date.now(), status: "Pending", date: new Date().toLocaleDateString(), ref: `CMP-${Date.now().toString().slice(-6)}` };
    setComplaints((prev) => [newComplaint, ...prev]);
    setForm({ name: "", dept: "", subject: "", desc: "" });
    addToast("Complaint registered", `Reference: ${newComplaint.ref}`, "success");
  }

  const inputClass = `w-full px-3 py-2.5 rounded-xl border text-sm outline-none focus:border-indigo-500 transition-all ${darkMode ? "bg-slate-800 border-slate-700 text-white placeholder-slate-400" : "bg-white border-slate-300"}`;
  ...
  return (
    <div className={`min-h-screen ${darkMode ? "bg-slate-950" : "bg-slate-50"} py-8 px-4`}>
      <div className="max-w-6xl mx-auto">
        <PageHeader title="Complaints & Feedback" subtitle="We take every complaint seriously and respond as quickly as possible." icon="feedback" />
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className={`p-6 rounded-2xl border shadow-lg ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              {[{ label: "Your Name *", key: "name", placeholder: "Full name" }, { label: "Department", key: "dept", placeholder: "e.g. OPD, ICU, Pharmacy" }, { label: "Subject *", key: "subject", placeholder: "Brief subject" }].map((field) => (
                <div key={field.key}>
                  <label className={labelClass}>{field.label}</label>
                  <input type="text" value={form[field.key]} onChange={(e) => setForm({ ...form, [field.key]: e.target.value })} placeholder={field.placeholder} className={inputClass} />
                </div>
              ))}
              <div>
                <label className={labelClass}>Description *</label>
                <textarea rows={4} value={form.desc} onChange={(e) => setForm({ ...form, desc: e.target.value })} placeholder="Describe your complaint..." className={`${inputClass} resize-none`} />
              </div>
              <ActionButton type="submit" label="Submit Complaint" variant="primary" size="md" />
            </form>
          </div>
          ...
          <div>
            ...
            {complaints.length === 0 ? (
              <div className={`text-center py-12 rounded-2xl border ${darkMode ? "bg-slate-900 border-slate-800 text-slate-400" : "bg-white border-slate-200 text-slate-500"}`}>No complaints registered</div>
            ) : (
