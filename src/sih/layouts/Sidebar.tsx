import { Activity, BarChart3, BrainCircuit, ShieldCheck, CalendarDays, ClipboardList, FlaskConical, LayoutDashboard, MonitorSmartphone, Pill, ReceiptText, ScanText, Settings, Stethoscope, UsersRound, X } from "lucide-react";
...
const groupByPath: Record<string, string> = { "/dashboard": "Operations", "/patients": "Clinical", "/doctors": "Clinical", "/appointments": "Clinical", "/laboratory": "Clinical", "/clinical-intelligence": "Clinical", "/pharmacy": "Clinical", "/medical-documents": "Clinical", "/billing": "Financial", "/reports": "Operations", "/admin": "Administration", "/physician-workspace": "Clinical" };
const iconByPath = { "/dashboard": LayoutDashboard, "/patients": UsersRound, "/doctors": Stethoscope, "/appointments": CalendarDays, "/laboratory": FlaskConical, "/clinical-intelligence": BrainCircuit, "/pharmacy": Pill, "/medical-documents": ScanText, "/billing": ReceiptText, "/reports": BarChart3, "/admin": Settings, "/physician-workspace": ClipboardList };
  ...
  return <>
    ...
    <aside aria-label="Primary navigation" className={`fixed inset-y-0 left-0 z-30 w-72 border-r border-slate-200 bg-white shadow-raised transition-transform duration-200 dark:border-slate-800 dark:bg-slate-950 ${open ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0`}>
      <div className="flex h-full flex-col p-5">
        ...
        <NavLink to="/kiosk" onClick={onClose} className="mb-2 flex min-h-10 items-center gap-3 rounded-lg border border-dashed border-cyan-300 px-3 py-2 text-sm font-medium text-cyan-800 transition hover:bg-cyan-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-600 dark:border-cyan-800 dark:text-cyan-200 dark:hover:bg-cyan-950/50"><MonitorSmartphone className="h-4 w-4" aria-hidden="true" />Start Kiosk Intake</NavLink>
        <NavLink to="/part6" onClick={onClose} className="mb-4 flex min-h-10 items-center gap-3 rounded-lg border border-dashed border-cyan-300 px-3 py-2 text-sm font-medium text-cyan-800 transition hover:bg-cyan-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-600 dark:border-cyan-800 dark:text-cyan-200 dark:hover:bg-cyan-950/50"><ShieldCheck className="h-4 w-4" aria-hidden="true" />Part 6 · ABDM / FHIR (demo)</NavLink>
