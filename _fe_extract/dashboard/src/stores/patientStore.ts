...
import { fetchPatients, fetchPatientById, createPatient as createPatientApi, updatePatient as updatePatientApi, deletePatient as deletePatientApi } from "../services/patientService.js";
...
export const usePatientStore = create<PatientStore>((set, get) => ({
  ...
  async addPatient(payload) {
    ...
    try {
      ...
      set((state) => ({ patients: [saved, ...state.patients], loading: false }));
    } catch (error) {
      // Backend unreachable or not deployed yet — keep the app usable locally,
      // but surface the failure rather than silently pretending it synced.
      set((state) => ({
  ...
  async updatePatient(id, payload) {
    set({ loading: true, error: null });
    try {
      const saved = await updatePatientApi(id, payload);
      set((state) => ({
        patients: state.patients.map((patient) => (patient.id === id ? saved : patient)),
        selectedPatient: state.selectedPatient?.id === id ? saved : state.selectedPatient,
        loading: false,
      }));
    } catch (error) {
      set((state) => ({
        patients: state.patients.map((patient) => (patient.id === id ? { ...patient, ...payload } : patient)),
        selectedPatient: state.selectedPatient?.id === id ? { ...state.selectedPatient, ...payload } : state.selectedPatient,
        loading: false,
        error: error instanceof Error ? `Saved locally only — ${error.message}` : "Saved locally only — could not reach the server",
      }));
    }
